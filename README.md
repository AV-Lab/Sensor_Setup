...
- To save synchronized image frames and pointcloud data , run the save_node node.
- Update  the topic names from [save_sample.yaml](sensors/config/save_sample.yaml)
- Other options such as synchronization threshold, sample folder, delay between frames, number of samples ...etc can also be selected from the config file.
- The format of the samples will be .png for image and .pcd for pointcloud.

sample synchronized data:
- refer to ## 📦 Working with ROS2 Bags on how and when to use use_sim_time
```bash
# From rosbag 
